...
class _Home_screenState extends State<Home_screen> {
  ...
  void dataaaa(double lat, double lon) async {
    print('ssaa ${lat} ${lon}');
  }
  ...
  void _onMarkerTapped(Marker marker) {
    _settingModalBottomSheet(map[marker.id]);
  }
  ...
  void _onMapCreated(GoogleMapController controller) async {
    ...
    for (int x = 0; x < positions_list.length; x++) {
      _controller
        ...
        icon: BitmapDescriptor.fromAsset("img/bus.png"),
        draggable: false,
        position: positions_list[x],
      ))
          .then((marker) {
        map[marker.id] = '${positions_list[x]}';
        return marker;
      });
    }
    _controller.onMarkerTapped.add(_onMarkerTapped);

    _controller.onMapTapped.add((LatLng loc) async {
       for (int x = 0; x < positions_list.length; x++) {


        distanceInKmBetweenEarthCoordinates(loc.latitude, loc.longitude,
            positions_list[x].latitude, positions_list[x].longitude);
      }

      currentLatLng = new LatLng(loc.latitude, loc.longitude);
      lat = loc.latitude;
      lon = loc.longitude;
      print('${lat} ${lon}  efefge');
      ...
      if (_marker != null) {
        ...
        _controller.moveCamera(CameraUpdate.newLatLngZoom(currentLatLng, 14));
      } else {
        _marker = await _controller.addMarker(MarkerOptions(
            position: currentLatLng,
            icon: BitmapDescriptor.defaultMarker,
            draggable: false));
